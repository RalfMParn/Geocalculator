...
class Puramiid(Tk):
    def __init__(self, switch_page_callback):
        ...
        self.title("Püramiid ruudu põhjaga Kalkulaator")
    ...
    def calc_and_display(self):
        ...
        while not valid_input:
            try:
                side_length = float(self.entry1.get())
                height = float(self.entry2.get())
                if side_length <= 0 or height <= 0:
                    raise ValueError("Külje pikkus ja kõrgus peab olema positiivne arv")
                valid_input = True
        ...
        if valid_input:
            side_length = float(self.entry1.get())
            height = float(self.entry2.get())
            base_area,  volume, side_area = self.model.calculate_square_pyramid_values(side_length, height)
            ...
            side_length = str(side_length).rstrip('.0')
            ...
            self.text_box.insert(END, f"Külje pikkus: {side_length}\nKõrgus: {height}\nPõhja pindala: {base_area}\nRuumala: {volume}\nKülgede pindala: {side_area}")
    ...
    def create_frame_widgets(self):
        ...
        self.selected_option.set(options[5])  # Set the default option
    ...
    def display_error(self, error):
        messagebox.showerror("Viga", f"Arv on vigane. Külje pikkus ja kõrgus peab olema positiivne arv. Error: {error}")
