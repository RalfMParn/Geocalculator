...
class Kolmnurk(Tk):
    def __init__(self, switch_page_callback):
        ...
        self.title("Täisnurkne kolmnurk Kalkulaator")
    ...
    def calc_and_display(self):
        ...
        while not valid_input:
            try:
                ...
                if base <= 0 or height <= 0:
                    raise ValueError("Alus ja kõrgus peab olema positiivne arv")
                valid_input = True
        ...
        if valid_input:
            base = float(self.entry1.get())
            height = float(self.entry2.get())
            hypotenuse, perimeter, area = self.model.calculate_rigth_triangle_values(base, height)
            ...
            self.text_box.insert(END, f"Alus: {base}\nKõrgus: {height}\nHüpotenuus: {hypotenuse}\nPerimeeter: {perimeter}\nPindala: {area}")
    ...
    def create_frame_widgets(self):
        ...
        self.lbl_info = Label(self.top_frame, text="Alus", bg="orange", font=self.default_Font)
        ...
        self.selected_option.set(options[6])  # Set the default option
    ...
    def display_error(self, error):
        messagebox.showerror("Viga", f"Arv on vigane. Alus ja kõrgus peab olema positiivne arv. Error: {error}")
