...
class Risttahukas(Tk):
    def __init__(self, switch_page_callback):
        super().__init__()
        self.switch_page_callback = switch_page_callback
        self.model = CalculatorModel()
        self.title("Risttahukas Kalkulaator")
        ...
        (self.btn_arvuta, self.lbl_info, self.lbl_info2, self.lbl_info3, self.entry1,
         self.entry2, self.entry3, self.drop, self.btn_select, self.text_box) = self.create_frame_widgets()
    ...
    def calc_and_display(self):
        ...
        while not valid_input:
            try:
                ...
                width = float(self.entry3.get())
                if radius <= 0 or height <= 0 or width <= 0:
                    raise ValueError("Raadius, laius ja kõrgus peab olema positiivne arv")
                valid_input = True
        ...
        if valid_input:
            length = float(self.entry1.get())
            width = float(self.entry3.get())
            height = float(self.entry2.get())
            diagonal, volume, total_surface_area = self.model.calculate_risttahukas_values(length, width, height)

            if isinstance(diagonal, float) and diagonal.is_integer():
                diagonal = int(diagonal)
            if isinstance(volume, float) and volume.is_integer():
                volume = int(volume)
            if isinstance(total_surface_area, float) and total_surface_area.is_integer():
                total_surface_area = int(total_surface_area)

            length = str(length).rstrip('.0')
            width = str(width).rstrip('.0')
            ...
            self.text_box.insert(END, f"Pikkus: {length}\nLaius: {width}\nKõrgus: {height}\n"
                                      f"Diagonaal: {diagonal}\nRuumala: {volume}\nfTäis pindala: {total_surface_area}")
    ...
    def create_frame_widgets(self):
        ...
        self.btn_arvuta.grid(row=2, column=3, padx=5, pady=5)
        ...
        self.lbl_info3 = Label(self.top_frame, text="Laius", bg="orange", font=self.default_Font)
        self.lbl_info3.grid(row=2, column=0, padx=5, pady=5)
        ...
        self.entry3 = Entry(self.top_frame, font=self.default_Font)
        ...
        self.selected_option.set(options[4])  # Set the default option
        ...
        return (self.btn_arvuta, self.lbl_info, self.lbl_info2, self.lbl_info3, self.entry1,
                self.entry2, self.entry3, self.drop, self.btn_select, self.text_box)

    def display_error(self, error):
        messagebox.showerror("Viga", f"Arv on vigane. Raadius, laius ja kõrgus peab olema positiivne arv. Error: {error}")
