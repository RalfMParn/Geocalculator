...
class Staadion(Tk):
    def __init__(self, switch_page_callback):
        ...
        self.title("Staadion Kalkulaator")
    ...
    def calc_and_display(self):
        ...
        if valid_input:
            value1 = float(self.entry1.get())
            value2 = float(self.entry2.get())
            perimeter, total_area, square_area = self.model.calculate_rigth_triangle_values(value1, value2)

            if isinstance(perimeter, float) and perimeter.is_integer():
                perimeter = int(perimeter)
            if isinstance(total_area, float) and total_area.is_integer():
                total_area = int(total_area)
            if isinstance(square_area, float) and square_area.is_integer():
                square_area = int(square_area)
            ...
            self.text_box.insert(END, f"Raadius: {value1}\nPikkus: {value2}\nÜmbermõõt: {perimeter}\nPindala: {total_area}\nPindala ilma otsteta: {square_area}")
    ...
    def create_frame_widgets(self):
        ...
        self.lbl_info2 = Label(self.top_frame, text="Pikkus", bg="orange", font=self.default_Font)
        ...
        self.selected_option.set(options[7])  # Set the default option
    ...
    def display_error(self, error):
        messagebox.showerror("Viga", f"Arv on vigane.Raadius ja Pikkus peab olema positiivne arv. Error: {error}")
